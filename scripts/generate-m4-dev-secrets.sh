...
files=(ingestion_migration_password ingestion_runtime_password ingestion_cleanup_password ingestion_e2e_password ingestion_migration_pgpass ingestion_runtime_dsn ingestion_e2e_dsn ingestion_e2e_container_dsn ingestion_cleanup_dsn ingestion_minio_access_key ingestion_minio_secret_key ingestion_cleanup_minio_access_key ingestion_cleanup_minio_secret_key ingestion_e2e_minio_access_key ingestion_e2e_minio_secret_key catalog_ingestion_rabbitmq_uri ingestion_rabbitmq_uri ingestion_e2e_rabbitmq_uri ingestion_e2e_rabbitmq_container_uri edge_status_rabbitmq_uri_1 edge_status_rabbitmq_uri_2)
...
jq --arg catalog_consume "$catalog_consume_password" --arg ingestion "$ingestion_password" --arg ingestion_e2e "$ingestion_e2e_rabbitmq_password" --arg edge1 "$edge_password_1" --arg edge2 "$edge_password_2" '
  .users += [
    {name:"catalog_ingestion",password:$catalog_consume,tags:""},
    {name:"ingestion_worker",password:$ingestion,tags:""},
    {name:"ingestion_e2e",password:$ingestion_e2e,tags:""},
  ...
  .queues += [
    ...
    {name:"ingestion.deletion.retry.5s",vhost:"/",durable:true,auto_delete:false,arguments:{"x-queue-type":"quorum","x-message-ttl":5000,"x-dead-letter-exchange":"raglibrarian.events.v1","x-dead-letter-routing-key":"catalog.book.deletion-requested.v1","x-dead-letter-strategy":"at-least-once","x-max-length-bytes":268435456,"x-overflow":"reject-publish"}},
    {name:"ingestion.deletion.retry.30s",vhost:"/",durable:true,auto_delete:false,arguments:{"x-queue-type":"quorum","x-message-ttl":30000,"x-dead-letter-exchange":"raglibrarian.events.v1","x-dead-letter-routing-key":"catalog.book.deletion-requested.v1","x-dead-letter-strategy":"at-least-once","x-max-length-bytes":268435456,"x-overflow":"reject-publish"}},
    {name:"ingestion.deletion.retry.2m",vhost:"/",durable:true,auto_delete:false,arguments:{"x-queue-type":"quorum","x-message-ttl":120000,"x-dead-letter-exchange":"raglibrarian.events.v1","x-dead-letter-routing-key":"catalog.book.deletion-requested.v1","x-dead-letter-strategy":"at-least-once","x-max-length-bytes":268435456,"x-overflow":"reject-publish"}},
  ...
  .bindings += [
    ...
    {source:"raglibrarian.ingestion.retry.v1",vhost:"/",destination:"ingestion.deletion.retry.5s",destination_type:"queue",routing_key:"ingestion.deletion.retry.5s",arguments:{}},
    {source:"raglibrarian.ingestion.retry.v1",vhost:"/",destination:"ingestion.deletion.retry.30s",destination_type:"queue",routing_key:"ingestion.deletion.retry.30s",arguments:{}},
    {source:"raglibrarian.ingestion.retry.v1",vhost:"/",destination:"ingestion.deletion.retry.2m",destination_type:"queue",routing_key:"ingestion.deletion.retry.2m",arguments:{}},
...
trap - EXIT
unset ingestion_migration_password ingestion_runtime_password ingestion_cleanup_password ingestion_e2e_password ingestion_minio_secret_key ingestion_cleanup_minio_secret_key ingestion_e2e_minio_secret_key catalog_consume_password ingestion_password ingestion_e2e_rabbitmq_password edge_password_1 edge_password_2
