...
func TestDecodeAcceptsSanitizedStatusEvent(t *testing.T) {
	payload, err := proto.Marshal(&catalogv1.BookProcessingStatusChangedV1{
		...
		ProcessingVersion: 3, UpdatedAt: timestamppb.New(time.Unix(10, 0)), Producer: "catalog-service", SchemaVersion: "v1",
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestDecodeRejectsSSEIdentifierInjection(t *testing.T) {
	payload, err := proto.Marshal(&catalogv1.BookProcessingStatusChangedV1{
		...
		ProcessingVersion: 1, UpdatedAt: timestamppb.Now(), Producer: "catalog-service", SchemaVersion: "v1",
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestValidFailureUsesCanonicalCategories(t *testing.T) {
	if !validFailure("encrypted_document") || validFailure("encrypted") {
