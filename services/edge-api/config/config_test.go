...
func TestLoadParsesExplicitSecurityConfiguration(t *testing.T) {
	...
	assert.Equal(t, time.Minute, cfg.QueryConcurrencyRetryAfter)
	assert.Equal(t, 2000, cfg.QueryMaxQuestionLength)
	assert.Equal(t, 20, cfg.QueryMaxTags)
	assert.Equal(t, 64, cfg.QueryMaxTagLength)
	assert.Equal(t, 256, cfg.QueryMaxAuthorLength)
	assert.Equal(t, 5, cfg.QueryDefaultLimit)
	assert.Equal(t, 20, cfg.QueryMaxLimit)
	assert.Equal(t, 25, cfg.AdminPendingPageDefaultSize)
...
func TestLoadParsesQueryAdmissionControls(t *testing.T) {
	...
	t.Setenv("EDGE_QUERY_CONCURRENCY_RETRY_AFTER", "75s")
	t.Setenv("EDGE_QUERY_MAX_QUESTION_LENGTH", "1500")
	t.Setenv("EDGE_QUERY_MAX_TAGS", "12")
	t.Setenv("EDGE_QUERY_MAX_TAG_LENGTH", "48")
	t.Setenv("EDGE_QUERY_MAX_AUTHOR_LENGTH", "180")
	t.Setenv("EDGE_QUERY_DEFAULT_LIMIT", "4")
	t.Setenv("EDGE_QUERY_MAX_LIMIT", "11")
	t.Setenv("EDGE_ADMIN_PENDING_PAGE_DEFAULT_SIZE", "12")
	t.Setenv("EDGE_ADMIN_PENDING_PAGE_MAX_SIZE", "120")
	t.Setenv("EDGE_BOOKS_PAGE_MAX_SIZE", "140")
	t.Setenv("EDGE_BOOKS_PAGE_TOKEN_MAX_SIZE", "640")
	...
	assert.Equal(t, 75*time.Second, cfg.QueryConcurrencyRetryAfter)
	assert.Equal(t, 1500, cfg.QueryMaxQuestionLength)
	assert.Equal(t, 12, cfg.QueryMaxTags)
	assert.Equal(t, 48, cfg.QueryMaxTagLength)
	assert.Equal(t, 180, cfg.QueryMaxAuthorLength)
	assert.Equal(t, 4, cfg.QueryDefaultLimit)
	assert.Equal(t, 11, cfg.QueryMaxLimit)
	assert.Equal(t, 12, cfg.AdminPendingPageDefaultSize)
