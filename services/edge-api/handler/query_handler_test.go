...
func TestQueryReturnsRetrievedEvidenceAndUsesTrustedPrincipal(t *testing.T) {
	...
	h := handler.NewQueryHandler(retrieval, 0.6, testQueryPolicy())
...
func TestQueryAnswerModeReturnsAnswerSegmentsAndTrustedEvidence(t *testing.T) {
	...
	h := handler.NewQueryHandler(retrieval, 0.6, testQueryPolicy(), handler.WithAnswer(answer, admission))
	recorder := httptest.NewRecorder()
...
func TestQuerySearchModeBypassesAnswerAndPreservesResponseShape(t *testing.T) {
	for _, body := range []string{`{"question":"q"}`, `{"question":"q","mode":""}`, `{"question":"q","mode":"search"}`} {
		t.Run(body, func(t *testing.T) {
			...
			answer := &answerStub{}
			h := handler.NewQueryHandler(retrieval, 0.6, testQueryPolicy(), handler.WithAnswer(answer, &answerAdmissionStub{allowed: true}))
			recorder := httptest.NewRecorder()
...
func TestQueryAnswerModePreservesEvidenceOnlyDegradation(t *testing.T) {
	...
	h := handler.NewQueryHandler(&retrievalStub{}, 0.6, testQueryPolicy(), handler.WithAnswer(answer, &answerAdmissionStub{allowed: true}))
	recorder := httptest.NewRecorder()
...
func TestQueryRequiresTrustedPrincipal(t *testing.T) {
	h := handler.NewQueryHandler(&retrievalStub{}, 0.6, testQueryPolicy())
...
func TestQueryHandlerRejectsTypedNilRetrieval(t *testing.T) {
	...
	assert.Panics(t, func() { handler.NewQueryHandler(retrieval, 0.6, testQueryPolicy()) })
}
...
func testQueryPolicy() handler.QueryPolicy {
	return handler.QueryPolicy{
