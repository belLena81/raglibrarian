...
func run(log *zap.Logger) error {
	...
	creds, err := clientCredentials(); if err != nil { return err }
	conn, err := grpc.NewClient(address, grpc.WithTransportCredentials(creds)); if err != nil { return err }; defer conn.Close()
...
func clientCredentials() (credentials.TransportCredentials, error) {
	...
	pool := x509.NewCertPool(); if !pool.AppendCertsFromPEM(ca) { return nil, errors.New("invalid internal CA") }
	cert, err := tls.LoadX509KeyPair(os.Getenv("EDGE_TLS_CERT_FILE"), os.Getenv("EDGE_TLS_KEY_FILE")); if err != nil { return nil, err }
	return credentials.NewTLS(&tls.Config{MinVersion: tls.VersionTLS13, RootCAs: pool, Certificates: []tls.Certificate{cert}, ServerName: "identity-service"}), nil
}
