...
type stubDispatcherRuntime struct{}

func (stubDispatcherRuntime) Dispatch(context.Context) error { return nil }
...
func TestGetRuntimeRetriesAfterInitializationFailure(t *testing.T) {
	...
	first, err := getRuntimeWithLoader(context.Background(), func(context.Context) (dispatcherRuntime, error) {
		loads++
	...
	second, err := getRuntimeWithLoader(context.Background(), func(context.Context) (dispatcherRuntime, error) {
		loads++
		return stubDispatcherRuntime{}, nil
	})
	if err != nil || second == nil || loads != 2 {
		t.Fatalf("second getRuntimeWithLoader() = %#v, %v loads=%d", second, err, loads)
	}
	third, err := getRuntimeWithLoader(context.Background(), func(context.Context) (dispatcherRuntime, error) {
		loads++
