...
type stubPlannerRuntime struct{}

func (stubPlannerRuntime) Plan(context.Context, lambdaadapter.RabbitEvent) error { return nil }
...
func TestGetRuntimeRetriesAfterInitializationFailure(t *testing.T) {
	...
	first, err := getRuntimeWithLoader(context.Background(), func(context.Context) (plannerRuntime, error) {
		loads++
	...
	second, err := getRuntimeWithLoader(context.Background(), func(context.Context) (plannerRuntime, error) {
		loads++
		return stubPlannerRuntime{}, nil
	})
	if err != nil || second == nil || loads != 2 {
		t.Fatalf("second getRuntimeWithLoader() = %#v, %v loads=%d", second, err, loads)
	}
	third, err := getRuntimeWithLoader(context.Background(), func(context.Context) (plannerRuntime, error) {
		loads++
