...
func (f *ProtoEventFactory) Started(source application.UploadedEvent, job domain.ProcessingJob, now time.Time) (application.OutboxEvent, error) {
	...
	message := &ingestionv1.BookProcessingStartedV1{EventId: id, BookId: source.BookID, SourceSha256: source.SourceSHA256[:], ExtractionVersion: extractor.ExtractionVersion, NormalizationVersion: chunking.NormalizationVersion, TokenizerVersion: chunking.TokenizerVersion, ChunkingVersion: chunking.ChunkingVersion, CorrelationId: source.CorrelationID, OccurredAt: timestamppb.New(now), CausationId: source.EventID, Producer: "ingestion-service", SchemaVersion: "v1", IdempotencyKey: fmt.Sprintf("%s:%s:started", source.BookID, job.ConfigDigest())}
...
func (f *ProtoEventFactory) Failed(source application.UploadedEvent, job domain.ProcessingJob, category domain.FailureCategory, now time.Time) (application.OutboxEvent, error) {
	...
	message := &ingestionv1.BookProcessingFailedV1{EventId: id, BookId: source.BookID, SourceSha256: source.SourceSHA256[:], ExtractionVersion: extractor.ExtractionVersion, NormalizationVersion: chunking.NormalizationVersion, TokenizerVersion: chunking.TokenizerVersion, ChunkingVersion: chunking.ChunkingVersion, FailureCategory: protoCategory, CorrelationId: source.CorrelationID, OccurredAt: timestamppb.New(now), CausationId: source.EventID, Producer: "ingestion-service", SchemaVersion: "v1", IdempotencyKey: fmt.Sprintf("%s:%s:failed", source.BookID, job.ConfigDigest())}
