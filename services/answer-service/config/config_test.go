...
func TestLoadUsesSecureBoundedDefaults(t *testing.T) {
	...
	if configuration.Generator.MaxRequestBytes != 128<<10 || configuration.Generator.MaxResponseBytes != 128<<10 || configuration.Generator.MaxCandidateBytes != 32<<10 {
		t.Fatalf("unexpected provider policy: request=%d response=%d candidate=%d", configuration.Generator.MaxRequestBytes, configuration.Generator.MaxResponseBytes, configuration.Generator.MaxCandidateBytes)
	}
	...
	if configuration.Cache.Capacity != 0 || configuration.Cache.TTL != 0 {
		t.Fatalf("cache must be disabled by default: %#v", configuration.Cache)
	}
...
func TestLoadDefaultsFreeTierProviderRateLimit(t *testing.T) {
	...
	if configuration.Generator.RequestsPerMinute != 15 {
		t.Fatalf("Provider.RequestsPerMinute = %d, want 15", configuration.Generator.RequestsPerMinute)
	}
}
...
func TestLoadOverridesProviderPolicy(t *testing.T) {
	...
	t.Setenv("ANSWER_PROVIDER_MAX_REQUEST_BYTES", "32768")
	...
	if configuration.Generator.MaxRequestBytes != 32768 || configuration.Generator.MaxResponseBytes != 65536 || configuration.Generator.MaxCandidateBytes != 16384 {
		t.Fatalf("unexpected provider policy: request=%d response=%d candidate=%d", configuration.Generator.MaxRequestBytes, configuration.Generator.MaxResponseBytes, configuration.Generator.MaxCandidateBytes)
	}
...
func TestLoadTreatsExplicitZeroProviderRateAsUnlimited(t *testing.T) {
	setRequiredEnvironment(t)
	t.Setenv("ANSWER_LLM_MODEL", "model:free")
	t.Setenv("ANSWER_PROVIDER_REQUESTS_PER_MINUTE", "0")
	configuration, err := Load()
	if err != nil {
		t.Fatal(err)
	}
...
func TestLoadRejectsInsecureProviderAndInvalidBounds(t *testing.T) {
	...
	setRequiredEnvironment(t)
	t.Setenv("ANSWER_PROVIDER_MAX_REQUEST_BYTES", "1048577")
	if _, err := Load(); err == nil {
		t.Fatal("oversized provider request limit accepted")
	}
	setRequiredEnvironment(t)
	...
	if _, err := Load(); err == nil {
		t.Fatal("partially enabled cache accepted")
	}
	setRequiredEnvironment(t)
	t.Setenv("ANSWER_CACHE_CAPACITY", "1")
