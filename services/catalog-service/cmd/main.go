...
func main() {
	...
	creds, err := serverCredentials(); if err != nil { panic(err) }
	grpcServer := grpc.NewServer(grpc.Creds(creds)); catalogv1.RegisterCatalogServiceServer(grpcServer, server{}); healthServer := health.NewServer(); healthServer.SetServingStatus("", grpc_health_v1.HealthCheckResponse_SERVING); grpc_health_v1.RegisterHealthServer(grpcServer, healthServer); if err = grpcServer.Serve(lis); err != nil { panic(err) }
}
func serverCredentials() (credentials.TransportCredentials, error) {
	...
	cert, err := tls.LoadX509KeyPair(os.Getenv("CATALOG_TLS_CERT_FILE"), os.Getenv("CATALOG_TLS_KEY_FILE")); if err != nil { return nil, err }
